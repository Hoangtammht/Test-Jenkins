        ...
        stage('Packaging/Pushing image') {
            ...
            steps {
                withDockerRegistry(credentialsId: 'dockerhub', url: 'https://index.docker.io/v1/') {
                    sh 'docker build -t hoangtammht/exeproject-image .'
                    sh 'docker push hoangtammht/exeproject-image'
                }
            }
        }
...
//         }
        ...
        stage('Deploy Spring Boot to DEV') {
            steps {
                echo 'Deploying and cleaning'
                sh 'docker image pull hoangtammht/exeproject-image'
                sh 'docker container stop hoangtammht/exeproject-image || echo "this container does not exist" '
                sh 'docker network create dev || echo "this network exists"'
                sh 'echo y | docker container prune '

                sh 'docker container run -d --rm --name hoangtammht/exeproject-image -p 8081:8080 --network dev hoangtammht/exeproject-image'
            }
        }
